...
def detect_statistical_edge(data, threshold=2.0, window=10, expected_under_rate=0.65):
    if len(data) < window:
        return {
            "rolling_avg": None,
            "under_count": None,
    ...
    edge_detected = rolling_avg < 1.5 and under_count >= 7 and z_score >= 1.5
    return {
...
# Streamlit App
def main():
    st.title("Crash Game Predictor with Statistical Edge Detection")
    ...
    if st.session_state.history:
        ...
        st.subheader("📊 Statistical Edge Detector")
        edge = detect_statistical_edge(data)
        if edge["rolling_avg"] is not None:
            st.write(f"Rolling Avg (Last 10): **{edge['rolling_avg']}x**")
            st.write(f"Under 2.0x Count: **{edge['under_count']}**")
            st.write(f"Z-Score (Volatility): **{edge['z_score']}**")
            if edge["edge_detected"]:
                st.success("✅ Edge Detected: Now may be a good time to bet 'Above'")
            else:
                st.info("⚠️ No clear edge detected — wait")
    ...
    st.caption("You start with 0.1 SOL. Fixed and flat bets only happen when the model predicts 'Above 2.0x'.")
